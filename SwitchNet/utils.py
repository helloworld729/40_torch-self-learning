...
def build_paragraph(text_data, my_vocab):  # text_data对应一篇文章
    #print(my_vocab[text_data[0]])
    #print(text_data)
    text_data = [text[:max_doc_len] for text in text_data]  # text_data是二维列表，每个元素代表一个句子
    indexs = [torch.tensor(my_vocab[text]).long() for text in text_data]  # my_vocab[text]返回text序列的索引
...
def filter_output(x, lengths):
    ...
    indexes = []
    for i in range(len(lengths)):  # i标志每一篇文章
...
def switch_within_para(mask, para_embed, para_len):
    ...
    origin_idx = list(cand_idx)  # 原始句子索引
    random.shuffle(cand_idx)     # 候选句子索引
    for i, idx in enumerate(cand_idx):
        if idx == origin_idx[i] and i!=0:  # 如果索引碰撞，候选索引交换
            cand_idx[i], cand_idx[i-1] = cand_idx[i-1], cand_idx[i]
        elif idx==origin_idx[i]:  # 如果索引碰撞，候选索引交换
            cand_idx[i], cand_idx[i+1] = cand_idx[i+1], cand_idx[i]
    new_cand_embed = list(para_embed)  # 拷贝文章
    new_cand_len = list(para_len)      # 拷贝长度
    for i, idx in enumerate(origin_idx):
        new_cand_embed[idx] = para_embed[cand_idx[i]]  # 拷贝文章的第idx句话，被替换为原文的cand_idx[i]句话
...
def switch_sentence(batch_data, sentence_cands):
    ...
    for para in batch_data:
        if len(para[0]) < 2:
            ...
            continue
        para_embed = list(para[0])  # index Tensor 列表
        para_len = list(para[1])    # 句子长度列表
        this_cand_pool_embed = []
        this_cand_pool_length = []
        mask = torch.rand(len(para_len))  # 生成para_len个0-1随机数
        mask = mask.le(mask_pro)
        if mask.sum() <=1:
            idx = list(range(len(mask)))
            sel_idx = random.sample(idx, 2)
            ...
            #mask[random.randint(0,len(mask)-1)]=1
        batch_mask.append(mask)
        para_embed, para_len = switch_within_para(mask, para_embed, para_len)  # 看不懂怎么交换的
        new_batch_data.append([para_embed, para_len])
