...
def build_training_pairs(outs, pool_sent_embeds, masks):  # mask任务使用
...
def train_switch(model, classification_layer,
    ...
    embeds = model(masked_paragraphs)  # batch， seq_len， hiddenSize*2
    #print(len(pos_score), len(neg_score))
    batch_size, doc_size, embed_dim = embeds.size()
    embeds = embeds.view(-1, embed_dim)
    sigmoid = nn.Sigmoid()
    scores = sigmoid(classification_layer(embeds).view(batch_size, doc_size))  # batch_size， doc_size即每句话打一个分
    #labels = masks.view(batch_size, doc_size).long().to(device)
    #labels = filter_output(labels.view(-1), paragraph_lengths)
    labels = torch.cat(masks).float().to(device)  # 实际有没有换
    scores = filter_output(scores.view(-1), paragraph_lengths)  # 整个批次的一维tesor  没看懂 Todo
    loss = loss_function(scores, labels)
    return loss
...
def train(train_data, dev_data, my_vocab, train_target, dev_target):
    ...
    all_paragraphs = [build_paragraph(this_sample, my_vocab)  # 返回元组列表，元组由两个列表构成：句子的索引tensor列表、句子长度列表
