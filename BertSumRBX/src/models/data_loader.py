...
class Batch(object):
    ...
    def __init__(self, data=None, device=None,  is_test=False):
        ...
        if data is not None:
            ...
            # mask = 1 - (src == 0)
            mask = ~(src == 0)  # padding的位置为False
            clss = torch.tensor(self._pad(pre_clss, -1))  # 根据最长句子数来
            # mask_cls = 1 - (clss == -1)
            mask_cls = ~(clss == -1)  # padding的位置为False
            clss[clss == -1] = 0
...
class Dataloader(object):
    ...
    def __iter__(self):
        for file in self.filesets_generator:
            fileData = DataIterator(args=self.args, dataset=file,
                         batch_size=self.batch_size, device=self.device,
                             shuffle=self.shuffle, is_test=self.is_test)

            for batch in fileData:
                yield batch
            gc.collect()
...
class DataIterator(object):
    ...
    def preprocess(self, ex, is_test):
        ...
        # src等长的seg序列
        segs = ex['segs']
        if not self.args.use_interval: segs=[0]*len(segs)
        # cls标记的位置
        clsPos = ex['clss']
        ...
        if is_test:
            return newsIndex, labels, segs, clsPos, src_txt, tgt_txt
        else:
            return newsIndex, labels, segs, clsPos
...
#         # self.cur_iter 初始化指向第一个文件
#         self.cur_iter = self._next_dataset_iterator(filesets_generator)
#
#         assert self.cur_iter is not None
#
#     def __iter__(self):
#         # datasets是语料库生成器，d是文件-->构成生成器
#         # 何时出发文件的切换呢？
#         datasets_iter = (d for d in self.filesets_generator)
#         # 外层用于控制文件切换
...
#
#             # 返回一个生成器
#             self.cur_iter = self._next_dataset_iterator(datasets_iter)
#
#     def _next_dataset_iterator(self, datasets_iter):
#         # 接受一个 生成器 作为参数
...
#             self.cur_dataset = next(datasets_iter)
...
if 1:
    ...
    parser.add_argument("-bert_data_path", default="../../../dataBert")
